...
async def handle_backup(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text("⏳ Memulai proses backup, Tunggu ya Sayang...")

    try:
        script_path = "/opt/julak-bot/backup_for_bot.sh"
...
async def route_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    trial_scripts = {
        "ssh_trial": ('SSH', '/opt/julak-bot/create_trial_ssh.sh'),
        "vmess_trial": ('VMESS', '/opt/julak-bot/create_trial_vmess.sh'),
        "vless_trial": ('VLESS', '/opt/julak-bot/create_trial_vless.sh'),
        "trojan_trial": ('TROJAN', '/opt/julak-bot/create_trial_trojan.sh'),
    }
    ...
    if command == "menu_running":
        await run_script_and_reply(update, context, ['sudo', '/opt/julak-bot/check_status_for_bot.sh'], "Status Layanan:")
        return ROUTE
    elif command == "menu_restart":
        await run_script_and_reply(update, context, ['sudo', '/opt/julak-bot/restart_for_bot.sh'], "Restart Layanan:")
        return ROUTE
        ...
        await run_script_and_reply(update, context, ['sudo', '/opt/julak-bot/trial_cleanup.sh'], "Trial Cleanup:")
        return ROUTE
        ...
        if command == "confirm_proceed":
            if action == 'reboot':
                ...
                await run_script_and_reply(update, context, ['sudo', '/opt/julak-bot/restore_for_bot.sh'], "Proses Restore:")
            else:
...
async def ssh_get_ip_limit_and_create(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await send_script_output(update, context, ['sudo', '/opt/julak-bot/create_ssh.sh', ud['username'], ud['password'], ud['duration'], ud['ip_limit']])
...
async def renew_ssh_get_duration_and_execute(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not duration.isdigit() or int(duration) <= 0:
        await update.message.reply_text("❌ Durasi harus berupa angka positif. Silakan coba lagi.")
        return RENEW_SSH_GET_DURATION
    await update.message.reply_text("⏳ Sedang memproses perpanjangan, Tunggu ya Sayang...")
    await send_script_output(update, context, ['sudo', '/opt/julak-bot/create_renew_ssh.sh', username, duration, str(update.effective_user.id)])
    context.user_data.clear()
    return ROUTE
...
async def renew_vmess_get_duration_and_execute(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not duration.isdigit() or int(duration) <= 0:
        await update.message.reply_text("❌ Durasi harus berupa angka positif. Silakan coba lagi.")
        return RENEW_VMESS_GET_DURATION
    await update.message.reply_text("⏳ Sedang memproses perpanjangan VMESS, Tunggu ya Sayang...")
    await send_script_output(update, context, ['sudo', '/opt/julak-bot/create_renew_vmess.sh', username, duration, str(update.effective_user.id)])
    context.user_data.clear()
    return ROUTE
...
async def ssh_select_account_and_show_config(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await send_script_output(update, context, ['sudo', '/opt/julak-bot/get_ssh_config.sh', user_input])
    context.user_data.clear()
    return ROUTE
...
async def create_ssh_trial_account(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await send_script_output(update, context, ['sudo', '/opt/julak-bot/create_trial_ssh.sh', duration])
...
async def create_trojan_trial_account(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await send_script_output(update, context, ['sudo', '/opt/julak-bot/create_trial_trojan.sh', duration])
    return ConversationHandler.END
...
async def vless_list_accounts(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        p = subprocess.run(
            ['sudo', '/opt/julak-bot/list_vless_users.sh'], 
...
async def trojan_get_quota_and_create(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    ud = context.user_data
    await send_script_output(update, context, ['sudo', '/opt/julak-bot/create_trojan_user.sh', ud['user'], ud['duration'], ud['ip_limit'], ud['quota_gb']])
    context.user_data.clear()
    return ROUTE
...
async def trojan_list_accounts(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        p = subprocess.run(
            ['sudo', '/opt/julak-bot/list_trojan_users.sh'], 
            capture_output=True, text=True, check=True, timeout=30
        )
        output = p.stdout.strip()
...
async def delete_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data == "confirm_proceed":
        ...
        try:
            script_map = {
                'ssh': '/opt/julak-bot/delete-ssh.sh',
                'vmess': '/opt/julak-bot/delete_vmess_user.sh',
                'vless': '/opt/julak-bot/delete_vless_user.sh',
                'trojan': '/opt/julak-bot/delete_trojan_user.sh'
            }
